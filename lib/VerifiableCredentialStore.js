...
const VALID_QUERY_KEYS = [
  'bundle', 'displayable', 'id', 'issuer', 'type'
];
...
export class VerifiableCredentialStore {
  ...
  constructor({edv, edvClient, capability, invocationSigner}) {
    ...
    // index to find bundle VCs of which a VC is a member
    edvClient.ensureIndex({attribute: ['meta.bundle']});
   ...
   *   `issuer`, `displayable`, and `bundle` filters.
   *
  ...
  async find({query} = {}) {
    ...
    for(const q of queries) {
      ...
      const {type, issuer, displayable, bundle} = q;
      const entry = {};
      if(type) {
   ...
   * @param {object} [meta={}] - Custom meta data to set; the `issuer` field
   *   will be auto-populated if not set in the custom `meta`.
   * @param {boolean} [updateMetaOnly=false] - Set to `true` to only update
   *   the `meta` data for a credential if it already exists.
   *
   * @returns {Promise<object>} - The stored EDV document.
   */
  async upsert({credential, meta = {}, updateMetaOnly = false}) {
    if(!(credential && typeof credential === 'object')) {
      throw new TypeError('"credential" must be an object.');
    }
