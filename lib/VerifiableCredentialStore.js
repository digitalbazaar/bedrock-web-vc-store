...
export class VerifiableCredentialStore {
   ...
   * @param {object} [meta={}] - Custom meta data to set; the `issuer` field
   *   will be auto-populated if not set in the custom `meta`.
   * @param {function} [mutator({doc})] - A function that is called if an
   *   existing credential is found and that must return the document to
   *   use to update the existing document.
   *
   * @returns {Promise<object>} - The stored EDV document.
   */
  async upsert({credential, meta = {}, mutator}) {
    if(!(credential && typeof credential === 'object')) {
      throw new TypeError('"credential" must be an object.');
    }
    ...
    while(true) {
      ...
      try {
        return await this.edvClient.upsert({doc});
      } catch(e) {
   ...
   *   `{deleted: boolean, doc}` where `doc` is only set if the
   *   deleted document was found.
   */
    ...
    try {
      ...
      if(bundleResult.allSubDocuments.length > 0) {
        if(deleteBundle) {
          ...
          await this._deleteBundledDocs(bundleResult);
        } else if(!force) {
          const error = new Error(
