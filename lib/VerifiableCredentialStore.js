...
export default class VerifiableCredentialStore {
   ...
   * @param {object} options.edv - An `EdvClient` instance to use.
   * @param {object} [options.capability] - An optional zcap with the authority
   *   to use the EDV's `/documents` API; if not provided, the root capability
   *   will be invoked when using the `EdvClient` instance.
   * @param {object} options.invocationSigner - The interface to use to invoke
   *   capabilities when using the `EdvClient` instance.
   */
  constructor({edv, capability, invocationSigner}) {
    this.edv = edv;
   ...
   */
  async get({id}) {
    const {capability, invocationSigner} = this;
  ...
  async find({query} = {}) {
    ...
    const {capability, invocationSigner} = this;
    const {documents: docs} = await this.edv.find({
  ...
  async delete({id}) {
    try {
      ...
      return this.edv.delete({doc, capability, invocationSigner});
    } catch(e) {
