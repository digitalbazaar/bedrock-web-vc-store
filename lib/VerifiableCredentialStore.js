...
export class VerifiableCredentialStore {
   ...
   *   document to use to update the existing document.
   * @param {array} [bundleContents=[]] - Optional bundle contents if the
   *   credential is a bundle of other credentials; each element is an object:
   *   `{credential, meta, [bundleContents], [dependent=true]}` where
   *   `bundleContents` is an optional set of sub-bundle contents and
   ...
   * @returns {Promise<object>} - The stored EDV document.
   */
  async upsert({credential, meta = {}, mutator, bundleContents}) {
    if(!(credential && typeof credential === 'object')) {
      throw new TypeError('"credential" must be an object.');
    }
    ...
    while(true) {
      ...
      try {
        result = await this.edvClient.upsert({doc});
        break;
      } catch(e) {
   ...
   *   found and `bundle` is only set if a bundle was deleted.
   */
  ...
  async _addBundleContents({bundleId, bundleContents}) {
    ...
    const mutator = _upsertBundleContentsMutator;
    const actions = bundleContents.map(entry => {
      ...
      actions.push(() => {
        ...
        set.add(bundleId);
        m.bundledBy = [...set];
        return this.upsert({credential, meta: m, mutator, bundleContents});
      });
    });
  ...
  async _deleteBundledDocs({bundle} = {}) {
    ...
    await this._runOps({ops: nonBundleOps, concurrency: 1, stopOnError: true});
  ...
  async _runOps({ops, concurrency = OPS_CONCURRENCY, stopOnError = false}) {
    ...
    for(const op of ops) {
      const {doc} = op;
      if(op.type === 'update') {
        actions.push(() => this.edvClient.upsert({doc}));
...
function _upsertBundleContentsMutator({doc, credential, meta}) {
