...
export class VerifiableCredentialStore {
   ...
   *
   * @param {string} [id] - The ID of the credential.
   * @param {object} [doc] - The decrypted EDV document for the credential.
   *
   ...
   *   `issuer`, `displayable`, and `bundledBy` filters.
   * @param {object} [options] - Query options such as `limit`.
   *
   ...
   * @param {array} [bundleContents=[]] - Optional bundle contents if the
   *   credential is a bundle of other credentials; each element is an object:
   ...
   * @param {object} [meta={}] - Custom meta data to set; the `issuer` field
   *   will be auto-populated if not set in the custom `meta`.
   * @param {function} [mutator({doc, credential, meta})] - A function that is
   *   called if an existing credential is found and that must return the
   *   document to use to update the existing document; the default function
   *   will not overwrite an existing `credential` and will try to safely
   *   merge `meta` fields, this can be disabled by passing `false` to
   *   overwrite both `credential` and `meta` completely or a custom function
   *   can be passed.
   * @param {array} [bundleContents=[]] - Optional bundle contents if the
   *   credential is a bundle of other credentials; each element is an object:
   *   `{credential, meta, [bundleContents], [dependent=true]}` where
  ...
  async upsert({
    ...
    assert.object(meta, 'meta');
    if(mutator) {
      // mutator may be falsey or a function
      if(typeof mutator !== 'function') {
        throw new TypeError('"mutator" must be falsey or a function.');
      }
    }
    meta = {...meta};
    ...
    while(true) {
      ...
      try {
        ...
        if(e.name === 'DuplicateError' && isNew) {
          await this.get({id: credential.id});
          // no exception, so credential was created while we were trying to
   ...
   *   credential.
   * @param {boolean} [deleteBundle=true] - If `true` and credential is a
   *   bundle, then any credentials that are part of it and depend on its
   *   existence (`meta.dependent=true`) will also be deleted.
   * @param {boolean} [force=false] - If `true` the credential will be forcibly
   *   deleted whether or not it is part of a bundle.
   *
  ...
  async _delete({id, docId, deleteBundle, force}) {
    ...
    try {
      ...
      if(!force && doc.meta.bundledBy && doc.meta.bundledBy.length > 0) {
