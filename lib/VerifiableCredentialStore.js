...
export default class VerifiableCredentialStore {
  constructor({hub, invocationSigner}) {
    ...
    this.hub.ensureIndex({attribute: ['content.issuer', 'content.type']});
    this.hub.ensureIndex({attribute: 'content.id', unique: true});
  }
  ...
  async find({type, issuer}) {
    ...
    if(issuer) {
      equals.push({'content.issuer': issuer});
    }
    const {invocationSigner} = this;
    const results = await this.hub.find({equals, invocationSigner});
  ...
  async delete({id}) {
    try {
      ...
      return this.hub.delete({id: doc.id, invocationSigner});
    } catch(e) {
